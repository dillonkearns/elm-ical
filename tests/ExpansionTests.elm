...
import Date
import Expect
import Ical.Expansion as Expansion
            ...
            [ test "event with no RRULE produces single occurrence" <|
                \() ->
                    let
                        ...
                        occurrences : List Expansion.Occurrence
                        occurrences =
                            Expansion.expand
            ...
            , test "DAILY INTERVAL=2 COUNT=3 produces every-other-day occurrences" <|
                \() ->
                    let
                        ...
                        occurrences : List Expansion.Occurrence
                        occurrences =
                            Expansion.expand yearRange event
                    in
                    occurrences
                        |> List.map (occurrenceDate Time.utc)
                        |> Expect.equal
...
occurrenceDate : Time.Zone -> Expansion.Occurrence -> Date.Date
